"""
python3 -m style_training.train_lora_instruct \
...
LLAMA3_INSTRUCT_PREPROMPT = """<|start_header_id|>system<|end_header_id|>
    ...
    * peft.get_peft_model(model, peft_config)
    ...
    def formatting_prompts_func(example):
        ...
        for i in range(len(example['original'])):
            text = f"{LLAMA3_INSTRUCT_POSTPROMPT}Rewrite the following:\nOriginal: {example['original'][i]}{LLAMA3_INSTRUCT_POSTPROMPT}\n\nRewrite: {example['rewrite'][i]} <|end_of_text|>"
